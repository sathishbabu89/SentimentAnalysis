...
HUGGINGFACE_API_TOKEN = "API"  # Replace with your actual Hugging Face API token
...
def main():
    ...
    # Main tabs
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8  = st.tabs(["Overview", "Channel Analysis", "Regional View", "Regional Map", "Issue Management", "Anomalies", "Executive Summary", "Social Media Insights"])
    ...
    with tab8:
        st.subheader("📢 Social Media Insights (Google)")
        st.markdown("""
        <div style="background-color: #f1f1f1; padding: 10px; border-radius: 8px;">
            Fetch real-time tweets mentioning <strong>Lloyds Bank</strong> via <a href="https://serpapi.com" target="_blank">SerpApi</a>
        </div>
        """, unsafe_allow_html=True)

        SERPAPI_KEY = "API"  # 🔁 Replace this with your actual key

        if st.button("🔄 Fetch Latest Feeds"):
            with st.spinner("Fetching feeds from Google via SerpApi..."):
                params = {
                    "engine": "google",
                    "q": "lloyds",
                ...
                try:
                    ...
                    if not tweets:
                        st.info("⚠️ No recent feeds found for 'Lloyds Bank'. Try again later or broaden your search.")
                    else:
                        st.success(f"✅ Fetched {len(tweets)} feeds")
                        for tweet in tweets[:10]:
                            with st.expander(tweet.get("title", "Feed")):
                                st.write(tweet.get("snippet", "No content available"))
                                if tweet.get("link"):
                                    st.markdown(f"[🔗 View Feed]({tweet['link']})")

                except Exception as e:
                    st.error(f"Failed to fetch feed: {str(e)}")
                    st.info("Please make sure your SerpApi key is correct and working.")
        else:
            st.info("Click the button above to fetch the latest tweets about Lloyds Bank.")
